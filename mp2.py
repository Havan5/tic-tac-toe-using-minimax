...
class GenGameBoard: 
    ...
    def makeCompMove(self):
        _, row, col = self.max_value()
        self.makeMove(row + 1, col + 1, 'O')

    # this run all the possible moves and retun the max position which is the best move for the AI
    def max_value(self):
        if self.is_terminal():
            return self.get_utility(), None, None
        
        v = -float('inf')
        for i, j in self.get_actions():
            self.marks[i][j] = 'O'
            v2, _, _ = self.min_value()
            self.marks[i][j] = ' '
            if v2 > v:
                v, row, col = v2, i, j
        return v, row, col
